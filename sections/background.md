...
should be *commutative*. To find a viable monoid then, we can search
for a function from the free commutative idempotent monoid on
occurrences to occurrences. Free idempotent commutative monoids are
well-studied: they are sets\todo{Cite a source for this}.

Informally, the two questions we have to answer to choose this
function are what the empty set should map to (which would be the identity
element for the monoid) and how to decide between a set of events with the same
time. To the first, it seems sensible to introduce another constructor to
`Future` for an occurrence infinitely far in the future. This is analogous to
construction of free monoids generated by semigroups\cite{semiToMonoid}.
